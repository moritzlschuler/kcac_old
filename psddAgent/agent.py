...
import shutil
import random
from psddAgent.prepareAgent import create_psdd_files
...
class psddAgent:
    ...
    def __init__(self, capacity_limits, state_size = 2*95+1, action_size = 95, seed=0, env_name = "BSS95-760", timesteps = 12, constraints = {}, \
        ...
        no_presampled_actions_per_file = 10_000, pooling = 1, device = 'cuda', no_threads = 4):
        """
        ...
        """
        if pooling > 1:
            NotImplementedError('Pooling is currently not correctly implemented, especially the psdd creation and uniform sampling')
        tc.manual_seed(seed)
        np.random.seed(seed)
        if device == 'cuda':
        ...
        self.zones2vars, psddFileName, self.presampledFileName =  self.readZoneVar(env_name, constraints, action_size, nresources)
        self.timesteps = timesteps
        self.capacity_limits = capacity_limits
        self.presampledFileNameBase = self.presampledFileName[:-4]
        ...
        self.no_presampled_actions_per_file = no_presampled_actions_per_file
        self.presample_uniform_actions(env_name)
    ...
    def readZoneVar(self, env_name, constraints, nzones, nresources):
        if not os.path.isdir(self.log_dir + 'psddAgentFiles/'):
            os.mkdir(self.log_dir + 'psddAgentFiles/')
        psddFileName = self.log_dir + "psddAgentFiles/psdds/" + env_name + ".psdd"
        if not os.path.isfile(psddFileName):
            print('Starting to create PSDD')
            with open(psddFileName, 'w') as fp: # needed to handle concurrent processes
                pass
            create_psdd_files(self.log_dir + "psddAgentFiles/", env_name, constraints, nzones, nresources)
            print('done')
        while True: # needed to handle concurrent processes
            if os.path.isfile(psddFileName[:-5] + '.done'):
                break
            sleep(1)

        zones2varsFileName = self.log_dir + "psddAgentFiles/zones2vars/zones2vars-" + env_name + ".pkl"

        presampledFileBaseName = self.log_dir + "psddAgentFiles/uniformActions/" + env_name + "/uniform_actions.pkl"

        zones2varsFile = open(zones2varsFileName, "rb")
        zones2vars=pickle.load(zones2varsFile)
        zones2varsFile.close()

        return zones2vars, psddFileName, presampledFileBaseName


    def presample_uniform_actions(self, name):
        uniform_actions_dir = self.log_dir + 'psddAgentFiles/uniformActions/'
        if not os.path.isdir(uniform_actions_dir):
            os.mkdir(uniform_actions_dir)
        uniform_actions_dir = self.log_dir + 'psddAgentFiles/uniformActions/' + name + '/'
        if not os.path.isdir(uniform_actions_dir):
            os.mkdir(uniform_actions_dir)
        print(self.uniform_action_sampling)
        for i in range(self.uniform_action_sampling):
            presampledFileName = self.presampledFileNameBase + str(i) + ".pkl"
            if not os.path.isfile(presampledFileName):
                pklfile = open(presampledFileName, 'ab')
                samples = []
                for j in range(self.no_presampled_actions_per_file):
                    sample_shape = tc.Size([1])
                    psdd_actions_uniform = self.sample_layer_uniform.sample(sample_shape)
                ...
                pklfile.close()
            print(i + 1, 'of', self.uniform_action_sampling)
            if i == self.uniform_action_sampling - 1:
                print('Uniform Actions are all presampled!')
                with open(presampledFileName[:-4] + '.done', 'w'):
                    pass
        while True: # needed to handle concurrent processes
            if os.path.isfile(presampledFileName[:-4] + '.done'):
                break
            sleep(1)
